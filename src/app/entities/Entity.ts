...
export class Entity {
	...
	async addChildEntity(child: Entity, entityTree: EntityTree, childType: string): Promise<any> {
		...
		await this.isomorphicGitService.fileWriteAndCommit("properties.json", JSON.stringify(childNode), child.name, true)
		...
		await this.isomorphicGitService.fileWriteAndCommit("properties.json", JSON.stringify(entityTree), "master", true)
